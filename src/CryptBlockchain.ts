class CryptBlockchain{
    blockchain: Array<CryptBlock>
    size: number

    constructor(id: number, blockchain?: CryptBlock[]){
        this.blockchain = [this.startGenesisBlock(id)]
        this.size = 1
        if(blockchain){
            this.size = 0
            this.blockchain = []
            blockchain.map((block: CryptBlock, index: number) => {
    ...
    addNewBlock(newBlock:CryptBlock){
